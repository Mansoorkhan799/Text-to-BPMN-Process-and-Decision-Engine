...
const LatexEditor: React.FC<LatexEditorProps> = ({ user, initialContent: initialContentProp, onContentChange, editorMode = 'code', onEditorModeChange, isSaving, onManualSave, projectId, onSaveComplete }) => {
    ...
    // Process LaTeX content for HTML preview (using Visual Editor's working approach)
    const processLatexContent = useCallback((latex: string) => {
        try {
            ...
            // Remove document setup commands (they shouldn't appear in preview)
            ...
            // Remove \tableofcontents command (it would be handled by LaTeX compiler)
            processedContent = processedContent.replace(/\\tableofcontents/g, '');

            // Remove \newpage commands (they would be handled by LaTeX compiler)
            processedContent = processedContent.replace(/\\newpage/g, '');
    ...
    const compileLatexWithContent = (contentToCompile: string) => {
        console.log('compileLatexWithContent called with content length:', contentToCompile.length);
        try {
            const processedHTML = processLatexContent(contentToCompile);
            console.log('Setting render output, length:', processedHTML.length);
            setRenderOutput(processedHTML);
            console.log('Render output set successfully');

        } catch (error) {
            console.error('Error compiling LaTeX:', error);
            setRenderOutput('<div class="error">Error compiling LaTeX</div>');
        }
    };

    const compileLatex = () => {
        compileLatexWithContent(latexContent);
    };
...
\\end{document}`;
    ...
    const insertTemplate = () => {
        if (editorInstance) {
            ...
            editorInstance.setValue(combinedContent);
            setLatexContent(combinedContent);
            ...
            // Force compile with the new content to update preview
            // Use setTimeout to ensure the editor content is set before compilation
            setTimeout(() => {
                console.log('Compiling template content, length:', combinedContent.length);
                compileLatexWithContent(combinedContent);
            ...
            if (e.ctrlKey || e.metaKey) {
                switch (e.key) {
                    ...
                    case 'i':
                        ...
                        return;
                    case 'Enter':
                        // Ctrl+Enter or Cmd+Enter for compile
                        e.preventDefault();
                        compileLatex();
                        return;
                }
