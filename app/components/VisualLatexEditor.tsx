...
const latexToSlate = (latexContent: string): Descendant[] => {
    ...
    try {
        ...
        // Remove document setup commands first
        let cleanedContent = latexContent
            ...
            .replace(/\\lfoot\s*\{.*?\}/g, '')
            .replace(/\\cfoot\s*\{.*?\}/g, '')
            .replace(/\\fancyhead\s*\[.*?\]\s*\{.*?\}/g, '')
            .replace(/\\fancyfoot\s*\[.*?\]\s*\{.*?\}/g, '')
            ...
            .replace(/\\newpage/g, '')
            // Remove title, author, date commands (but extract them for metadata)
            .replace(/\\title\s*\{.*?\}/g, '')
            .replace(/\\author\s*\{.*?\}/g, '')
            .replace(/\\date\s*\{.*?\}/g, '')
            // Remove comments
        ...
        // Extract metadata BEFORE cleaning (so we don't lose this information)
        ...
        // Extract author from original content
        const authorMatch = latexContent.match(/\\author\{(.*?)\}/);
        if (authorMatch && authorMatch[1]) {
            author = authorMatch[1];
        }

        // Extract date from original content
        ...
        // Use cleaned content as the content to process (begin/end document already removed)
...
const VisualLatexEditor = ({ initialLatexContent, onContentChange, editorMode, onEditorModeChange, isSaving, onManualSave, projectId, user, onSaveComplete }: VisualLatexEditorProps = {}) => {
    ...
    const processLatexContent = useCallback((latex: string) => {
        try {
            // Get the title, author, and date if present (BEFORE cleaning)
            ...
            // Remove document setup commands (they shouldn't appear in visual editor)
            ...
            // Extract content (after cleaning, there won't be begin/end document tags)
            const documentContent = cleanedLatex;

            // Process maketitle command (check if title exists to show title section)
            let processedContent = documentContent;
            let hasTitleSection = title !== '';

            // Remove \maketitle command if it exists (it's handled by the title section)
            processedContent = processedContent.replace(/\\maketitle/g, '');

            // Add table of contents and page breaks back if they were in the original
            if (latex.includes('\\tableofcontents')) {
                processedContent = '<div class="tableofcontents"><h2>Table of Contents</h2><p><em>Table of contents will be generated automatically when document is compiled.</em></p></div>' + processedContent;
            }

            // Process remaining content for any missed commands
    ...
    const PROCESS_TEMPLATE = `\\documentclass[12pt]{article}
...
\\fancyhf{}
\\rhead{\\thepage}
\\lhead{Process Management Document}

\\title{Process Management Document}
...
\\end{document}`;
    ...
    const insertTemplate = () => {
        try {
            // Get current content in LaTeX format
            const currentLatexContent = slateToLatex(editor.children);
            
            // Combine current content with template content
            const combinedContent = currentLatexContent + '\n\n' + PROCESS_TEMPLATE;
            
            // Convert combined LaTeX to Slate format
            const slateContent = latexToSlate(combinedContent);
            
            // Replace editor content with combined content
            editor.children = slateContent;
            
            // Update the latexCode state to trigger preview update
            setLatexCode(combinedContent);
            
            // Trigger content change callback with combined LaTeX version
            if (onContentChange) {
            ...
            toast.success('Process Management Template added successfully!');
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e1e] text-white">
            ...
            <div className="toolbar-container bg-[#252a36] border-gray-700 flex-shrink-0">
                ...
                <div className="flex items-center px-3 py-2 flex-wrap gap-2">
                    ...
                    <button
                        ...
                        className="p-2 rounded bg-[#1a1f2e] text-white hover:bg-[#2a304a] transition"
                        title="Use Process Management Template"
                    >
                        <FaClone size={14} />
                    </button>
            ...
            <style jsx global>{`
                ...
                .latex-preview .tableofcontents h2 {
                  margin-top: 0;
                  margin-bottom: 8px;
                  font-size: 20px;
                }
                
                .latex-preview .tableofcontents p {
                  margin-bottom: 0;
                  font-size: 14px;
                  color: #6c757d;
                }
                
                .latex-preview .page-break {
                  height: 1px;
                ...
                .latex-preview .page-break::after {
                  content: "Page Break";
                  position: absolute;
                  top: -10px;
                  left: 50%;
