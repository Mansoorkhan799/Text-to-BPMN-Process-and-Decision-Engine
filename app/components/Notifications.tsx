...
const Notifications = ({ userRole = 'user' }: NotificationsProps) => {
    ...
    return (
        <div className="container mx-auto py-6 px-4">
            ...
            {/* BPMN Viewer Modal */}
            {showBpmnViewer && selectedNotification && (
                    ...
                    <div className="bg-white rounded-lg shadow-lg w-full max-w-6xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="p-4 border-t flex justify-between gap-2">
                            ...
                            <div className="flex gap-2">
                                ...
                                {canApproveReject && getNotificationStatus(selectedNotification) === 'pending' && selectedNotification.type === 'approval_request' && (
                                    <>
                                        <button
                                            onClick={() => handleApproveReject(selectedNotification.id, 'approved')}
                                            disabled={processingState !== null}
                                            className="px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 flex items-center"
                                        >
                                            {processingState?.id === selectedNotification.id && processingState.action === 'approved' ? (
                                                <>
                                        ...
                                        <button
                                            onClick={() => handleApproveReject(selectedNotification.id, 'rejected')}
                                            disabled={processingState !== null}
                                            className="px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 flex items-center"
                                        >
                                            {processingState?.id === selectedNotification.id && processingState.action === 'rejected' ? (
                                                <>
                                                    <svg className="animate-spin h-4 w-4 mr-1.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
